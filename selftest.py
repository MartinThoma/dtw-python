...
def crossvalidation():
    ...
    for dataset in datasets:
        sql = (
            "SELECT id, data FROM `wm_raw_draw_data` "
    ...
    for testset in range(K_FOLD):
        classification_accuracy.append({"correct": 0, "wrong": 0, "c10": 0, "w10": 0})
        for testdata in cv[testset]:
            ...
            results = classify(
                datasets, A, EPSILON, THRESHOLD, FLATTEN, SPACE_EVENLY, POINTS
            )
            end = time.time()
            ...
            if len(results) == 0:
                ...
                # high.
                logging.debug(
                    "Raw_data_id = %i as testdata got no results" % testdata["id"]
                )
            else:
            ...
            if answer_id == testdata["formula_id"]:
                ...
                logging.warning(
                    (
                        "Got raw_data_id %i wrong. "
                        "It is %i, but I thought it would be %i."
                    )
                    % (testdata["id"], testdata["formula_id"], answer_id)
                )
        ...
        classification_accuracy[testset]["accuracy"] = float(
            classification_accuracy[testset]["correct"]
        ) / (
            classification_accuracy[testset]["correct"]
            + classification_accuracy[testset]["wrong"]
        )
        classification_accuracy[testset]["a10"] = float(
    ...
    print(
        "* Top-1-Classification (%i-fold cross-validated): %0.5f"
        % (K_FOLD, (t1sum / K_FOLD))
    )
    print(
        "* Top-10-Classification (%i-fold cross-validated): %0.5f"
